...
namespace JocysCom.ClassLibrary.Runtime
{
	...
	public static partial class Attributes
	{
...
#if NETFRAMEWORK
		public static string GetDescription(object o, bool cache = true, string expr = null)
#else
		public static string GetDescription(object o, bool cache = true, [CallerArgumentExpression(nameof(o))] string expr = null)
#endif
		{
			if (o is null)
				return null;
			var type = o.GetType();
			if (!cache)
				return _GetDescription(o, expr);
			// If enumeration then use value as a key, otherwise use type string.
			var isExpression = o is string && expr?.Contains('.') == true;
			var key = isExpression
				? expr
				: type.IsEnum
					? o
					: type.ToString();
			return Descriptions.GetOrAdd(key, x => _GetDescription(x, expr));
		}
		...
		/// </summary>
		private static string _GetDescription(object o, string expr = null)
		{
#if !NETFRAMEWORK
			if (o is string && expr?.Contains('.') == true)
				return _GetDesciptionFromExpression(expr);
#endif
			return _GetDesciptionFromObject(o);
		}

#if !NETFRAMEWORK
		private static string _GetDesciptionFromExpression(string expr)
		{
			if (expr.StartsWith("nameof(") && expr.EndsWith(")"))
			{
				// nameof() expression
				expr = expr[7..^1].Trim(); // Remove "nameof(" and ")"
			}
			var lastDot = expr.LastIndexOf('.');
			// ParameterNames  (may be fully-qualified)
			var typeName = expr[..lastDot].Trim();
			// AssemblyPath
			var fieldName = expr[(lastDot + 1)..].Trim();
			// Find the Type — first try a fast direct lookup, then scan loaded assemblies.
			...
			if (type == null)
				return expr + " - type not found!";
			var member = type
				.GetMember(fieldName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance);
			if (member.Length > 0)
				return member[0].GetCustomAttribute<DescriptionAttribute>()?.Description ?? expr + " - member description not found!";
			return expr + " - member not found";
		}

#endif

		private static string _GetDesciptionFromObject(object o)
		{
			var type = o.GetType();
